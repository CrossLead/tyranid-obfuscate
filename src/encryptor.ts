...
export class AesUtil implements Encryptor {
    ...
    public encryptString(input: string ): string {
        ...
        // For now don't waut to deal with auth tags
        const cipher: crypto.Cipher = crypto.createCipher(this.algo, this.masterKey);
        ...
        var encryptedText = cipher.update(input, 'utf8', 'hex')
    ...
    public decrypt (encdata: string): string{
        ...
        /** No Auth Tag Solution, not as secure */
        const decipher = crypto.createDecipher(this.algo, this.masterKey);
